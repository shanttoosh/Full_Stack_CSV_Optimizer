...
function initializeApp() {
    ...
    // Force responsive layout for layer selection
    forceResponsiveLayout();

    console.log('CSV Chunking Optimizer initialized successfully!');
}

// Force responsive layout for the front page
function forceResponsiveLayout() {
    const layerSelection = document.querySelector('.layer-selection');
    if (layerSelection) {
        // Force grid layout
        layerSelection.style.display = 'grid';
        layerSelection.style.gridTemplateColumns = 'repeat(auto-fit, minmax(280px, 1fr))';
        layerSelection.style.gap = 'clamp(15px, 3vw, 25px)';
        layerSelection.style.width = '100%';
        layerSelection.style.maxWidth = '100%';
        layerSelection.style.boxSizing = 'border-box';
        
        // For screens wider than 1200px, use 3 equal columns
        if (window.innerWidth >= 1200) {
            layerSelection.style.gridTemplateColumns = 'repeat(3, 1fr)';
            layerSelection.style.gap = '25px';
        }
        
        console.log('Responsive layout applied to layer selection');
    }
    
    // Also apply to layer cards
    const layerCards = document.querySelectorAll('.layer-card');
    layerCards.forEach(card => {
...
function setupMobileSidebarToggle() {
    ...
    if (toggleButton && sidebar) {
        ...
        window.addEventListener('resize', function() {
            ...
            // Re-apply responsive layout on resize
            forceResponsiveLayout();
        });
    }
}
