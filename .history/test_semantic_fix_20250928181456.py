...
10,1,2,"Nasser, Mrs. Nicholas",female,14"""
        ...
        if response.status_code == 200:
            ...
            if 'processing_summary' in result:
                ...
                print(f"🔍 Processing summary keys: {list(summary.keys())}")
                
                # Check different possible locations for chunking info
                chunking_method = None
                if 'chunking_results' in summary:
                    chunking_method = summary['chunking_results'].get('method', 'unknown')
                elif 'chunking_settings' in summary:
                    chunking_method = summary['chunking_settings'].get('method', 'unknown')
                elif 'chunking' in summary:
                    chunking_method = summary['chunking'].get('method', 'unknown')
                elif 'settings' in summary and 'chunking' in summary['settings']:
                    chunking_method = summary['settings']['chunking'].get('method', 'unknown')
                ...
                if chunking_method:
                    ...
                    print("⚠️ Could not find chunking method in response")
                    return False
            else:
