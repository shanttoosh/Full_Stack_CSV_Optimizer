...
class ProcessingPipeline:
    ...
    async def process_csv(self, 
        ...
        try:
            ...
            print(f"🔍 [PIPELINE] Step 1: Validating CSV data for {filename}")
            df = validate_csv_data(csv_data)
            print(f"✅ [PIPELINE] CSV validated: {df.shape[0]} rows, {df.shape[1]} columns")
            print(f"🔍 [PIPELINE] CSV columns: {list(df.columns)}")
            
            # Step 2: Merge settings
            print(f"🔍 [PIPELINE] Step 2: Getting {layer_mode} mode settings")
            base_settings = get_layer_defaults(layer_mode)
            if custom_settings:
                settings = merge_settings(base_settings, custom_settings)
            else:
                settings = base_settings
            print(f"✅ [PIPELINE] Settings configured: {settings}")
            
            # Validate settings
            validated_settings = validate_processing_settings(settings)
            print(f"✅ [PIPELINE] Settings validated")
            
            # Step 3: Preprocessing
            print(f"🔍 [PIPELINE] Step 3: Running preprocessing")
            ...
            print(f"✅ [PIPELINE] Preprocessing complete: {processed_df.shape[0]} rows, {processed_df.shape[1]} columns")
            
            # Step 4: Chunking
            print(f"🔍 [PIPELINE] Step 4: Running {validated_settings.get('chunking', {}).get('method', 'unknown')} chunking")
            chunking_result = await self._run_chunking(processed_df, validated_settings.get("chunking", {}))
            print(f"✅ [PIPELINE] Chunking complete: {len(chunking_result.chunks) if chunking_result and chunking_result.chunks else 0} chunks created")
            
            # Step 5: Embedding
            print(f"🔍 [PIPELINE] Step 5: Generating embeddings with {validated_settings.get('embedding', {}).get('model', 'unknown')}")
            embedding_result = await self._run_embedding(
                chunking_result, 
                validated_settings.get("embedding", {}),
                filename
            )
            print(f"✅ [PIPELINE] Embeddings complete: {embedding_result.total_chunks if embedding_result else 0} embeddings, model: {embedding_result.model_used if embedding_result else 'unknown'}")
    ...
    async def _run_chunking(self, df: pd.DataFrame, settings: Dict[str, Any]):
        ...
        print(f"🔍 [CHUNKING] Method: {method}, Settings: {settings}")
        
        try:
            loop = asyncio.get_event_loop()
            # Extract method-specific parameters
            method_params = {k: v for k, v in settings.items() if k != "method"}
            print(f"🔍 [CHUNKING] Method params: {method_params}")
            ...
            print(f"❌ [CHUNKING] Chunking failed with error: {e}")
            import traceback
            traceback.print_exc()
            print(f"⚠️ [CHUNKING] Falling back to fixed chunking")
