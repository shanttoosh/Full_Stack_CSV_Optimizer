...
async function processDynamicStepByStep(file) {
    ...
    try {
        ...
        // Step 4: Storing
        console.log('📊 Step 4: Dynamic Storing');
        ...
        const storingResult = await apiClient.processStepStoring(file);
        const storingTime = stopStepTimer('step-storage', 'Storing');
        updateStepStatus('step-storage', 'completed');
        totalProcessingTime += storingResult.processing_time_seconds;
        allResults.storing = storingResult;
        
        console.log(`✅ Storing completed in ${storingResult.processing_time_seconds}s`);
        
        // Step 5: Retrieval Setup
        console.log('📊 Step 5: Retrieval Setup');
        startStepTimer('step-retrieval', 'Retrieval Setup');
        updateStepStatus('step-retrieval', 'active');
        if (progressText) progressText.textContent = 'Setting up Retrieval System...';
        
        // Simulate retrieval setup (very fast)
        await new Promise(resolve => setTimeout(resolve, 500));
        const retrievalTime = stopStepTimer('step-retrieval', 'Retrieval Setup');
        updateStepStatus('step-retrieval', 'completed');
        
        console.log(`✅ Retrieval setup completed in ${retrievalTime}s`);
