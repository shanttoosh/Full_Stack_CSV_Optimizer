...
async function startProcessing() {
    ...
    try {
        ...
        if (currentLayer === 1) {
            ...
            console.log('🔧 Default: Header validation, whitespace removal, encoding validation');
            
            console.log('🔍 Calling API with file:', uploadedFile.name, uploadedFile.size);
...
async function handleRealProcessingSuccess(result) {
    ...
    // Update all steps as completed with visual progression (skip analysis)
    const steps = ['step-preprocess', 'step-chunking', 'step-embedding', 'step-storage', 'step-retrieval'];
    const stepNames = [
        'Preprocessing (Header validation, encoding, whitespace)',
        'Semantic Chunking (5 clusters)', 
        'Generating Embeddings (all-MiniLM-L6-v2)',
        'Storing in ChromaDB (Cosine similarity)',
        'Retrieval System Ready'
    ];
    
    for (let i = 0; i < steps.length; i++) {
        // Show step as active first
        updateStepStatus(steps[i], 'active');
        if (progressText) {
            progressText.textContent = `Processing: ${stepNames[i]}`;
        }
        
        // Wait to show the active state
