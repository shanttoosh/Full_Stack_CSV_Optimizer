...
async function handleRealProcessingSuccess(result) {
    ...
    // Enable query interface with REAL search
    try {
        console.log('🔍 Processing ID:', result.processing_id);
        enableRealQueryInterface(result.processing_id);
        console.log('✅ Query interface enabled');
    } catch (error) {
        console.error('❌ Error enabling query interface:', error);
    }
...
async function processDynamicStepByStep(file) {
    ...
    try {
        ...
        // Step 4 & 5: Real Backend Processing (Storing + Retrieval)
        console.log('📊 Step 4-5: Real Backend Processing (Storing + Retrieval)');
        ...
        // Start retrieval timer as well since they happen together in backend
        startStepTimer('step-retrieval', 'Retrieval Setup');
        updateStepStatus('step-retrieval', 'active');
        
        // Call the real Layer 1 API which does actual storing and retrieval
        const realBackendStart = Date.now();
        const fullResult = await apiClient.processLayer1(file);
        const realBackendTime = (Date.now() - realBackendStart) / 1000;
        
        // Stop timers and update with real timing
        const storingTime = stopStepTimer('step-storage', 'Storing');
        const retrievalTime = stopStepTimer('step-retrieval', 'Retrieval Setup');
        
        updateStepStatus('step-storage', 'completed');
        updateStepStatus('step-retrieval', 'completed');
        
        // Use real backend time for both steps (they happen together)
        const halfTime = realBackendTime / 2;
        totalProcessingTime += realBackendTime;
        
        // Update the timing displays with actual time
        const storageElement = document.getElementById('timing-storage');
        const retrievalElement = document.getElementById('timing-retrieval');
        if (storageElement) storageElement.textContent = `${Math.ceil(halfTime)}s`;
        if (retrievalElement) retrievalElement.textContent = `${Math.floor(halfTime)}s`;
        
        console.log(`✅ Real backend processing completed in ${realBackendTime.toFixed(2)}s`);
        
        // Store the full result for later use
        allResults.fullBackendResult = fullResult;
        ...
        const consolidatedResult = {
            ...
            processing_summary: {
                ...
                storage_results: {
                    storage_type: storingResult.storage_type,
                    similarity_metric: storingResult.similarity_metric
                },
                performance_metrics: {
        ...
        // Now call the full Layer 1 API to get actual download links and search capability
        console.log('📁 Getting actual download links from full Layer 1 API...');
        
        try {
            // Start timing for the real backend processing
            const realProcessingStart = Date.now();
            
            const fullResult = await apiClient.processLayer1(file);
            
            const realProcessingTime = (Date.now() - realProcessingStart) / 1000;
            console.log(`🕐 Real backend processing took: ${realProcessingTime.toFixed(2)}s`);
            
            if (fullResult && fullResult.success) {
                console.log('✅ Got full API result with download links:', fullResult);
                
                // Update the timing displays with real processing time breakdown
                const summary = fullResult.processing_summary || {};
                const realTiming = summary.step_timings || {};
                
                // Update step timings with real data if available
                if (realTiming.storing_seconds) {
                    const storageElement = document.getElementById('timing-storage');
                    if (storageElement) {
                        storageElement.textContent = `${realTiming.storing_seconds}s`;
                    }
                }
                
                if (realTiming.retrieval_seconds) {
                    const retrievalElement = document.getElementById('timing-retrieval');
                    if (retrievalElement) {
                        retrievalElement.textContent = `${realTiming.retrieval_seconds}s`;
                    }
                }
                
                // Show real download buttons
                if (fullResult.download_links) {
                    showRealDownloadButtons(fullResult.download_links);
                }
                
                // Enable real search interface
                if (fullResult.processing_id) {
                    enableRealQueryInterface(fullResult.processing_id);
                }
                
                // Update the consolidated result with real data
                consolidatedResult.download_links = fullResult.download_links;
                consolidatedResult.search_endpoint = fullResult.search_endpoint;
                consolidatedResult.processing_id = fullResult.processing_id;
                
            } else {
                console.warn('⚠️ Full API call failed, showing basic completion message');
                if (progressText) {
                    progressText.textContent = 'Processing completed - check console for details';
                }
            }
        } catch (error) {
            console.error('❌ Failed to get download links from full API:', error);
            if (progressText) {
...
function showRealDownloadButtons(downloadLinks) {
    ...
    if (!downloadSection) {
        ...
        downloadSection.innerHTML = `
            ...
            <div id="download-buttons" class="form-group" style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: center;"></div>
        `;
