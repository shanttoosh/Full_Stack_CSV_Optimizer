...
function updateStepStatus(stepId, status) {
    ...
    const statusElement = step.querySelector('.step-status') || step.querySelector('.process-status');
    ...
    switch (status) {
        case 'active':
            ...
            // Clear both elements when starting
            if (statusElement) {
...
function updateStepLiveTime(stepId, seconds) {
    ...
    if (stepElement) {
        const statusElement = stepElement.querySelector('.process-status') || 
                            stepElement.querySelector('.step-status');
        if (statusElement && stepElement.classList.contains('active')) {
            statusElement.textContent = `Processing... ${seconds}s`;
        }
    }
}
...
function updateStepCompletionTime(stepId, seconds, stepName) {
    const stepElement = document.getElementById(stepId);
    if (stepElement) {
        // Use step-timing for final time (grey text)
        const timingElement = document.getElementById(`timing-${stepId.replace('step-', '')}`);
        if (timingElement) {
            timingElement.textContent = `${seconds}s`;
        }
        
        // Use step-status for completion message (green text, no overlap)
