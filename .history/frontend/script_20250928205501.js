...
async function processDynamicStepByStep(file) {
    ...
    try {
        ...
        // Show download buttons immediately (skip the old static progression)
        console.log('📁 Showing download buttons for dynamic processing...');
        
        // For now, show a message that dynamic processing is complete
        // In a full implementation, you'd call the original Layer1 API to get actual download links
        const downloadMessage = {
            chunks_csv: { message: "Dynamic processing complete - use full API for downloads" },
            embeddings_json: { message: "Dynamic processing complete - use full API for downloads" },
            metadata_json: { message: "Dynamic processing complete - use full API for downloads" },
            results_zip: { message: "Dynamic processing complete - use full API for downloads" }
        };
        
        showRealDownloadButtons(downloadMessage);
        
        // Enable search interface
        enableRealQueryInterface(consolidatedResult.processing_id);
