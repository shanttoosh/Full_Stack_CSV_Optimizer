...
async function handleRealProcessingSuccess(result) {
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        // Wait a bit to show the active state
    ...
    // Complete processing with real stats
    if (progressText) {
        const summary = result.processing_summary;
        const statsText = `Processing Complete! ${summary?.chunking_results?.total_chunks || 0} chunks, ${summary?.embedding_results?.total_embeddings || 0} embeddings in ${processingTime.toFixed(1)}s`;
    ...
    // Show search section only if real query interface wasn't created
    setTimeout(() => {
        ...
        if (!realSearchSection) {
            ...
            console.log('✅ Real search interface already available');
        }
    }, 1000);
