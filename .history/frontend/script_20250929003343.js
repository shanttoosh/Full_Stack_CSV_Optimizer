...
async function processDynamicStepByStep(file) {
    ...
    try {
        ...
        // Step 4-5: Real Backend Processing (Storing + Retrieval + Downloads)
        console.log('📊 Step 4-5: Real Backend Processing - Storing, Retrieval & Downloads');
        
        // Start both timers since the backend does both together
        startStepTimer('step-storage', 'Storing');
        updateStepStatus('step-storage', 'active');
        if (progressText) progressText.textContent = 'Processing: Real-time Vector Storage & Retrieval...';
        
        // Also start retrieval timer (they happen together in the real backend)
        startStepTimer('step-retrieval', 'Retrieval Setup');
        updateStepStatus('step-retrieval', 'active');
        
        // Call the REAL Layer 1 API which does actual storing, retrieval, and creates download files
        console.log('📁 Calling REAL Layer 1 API for complete backend processing...');
        const realFullResult = await apiClient.processLayer1(file);
        
        // Stop timers with real processing time
        ...
        console.log(`✅ REAL backend processing completed in ${backendProcessingTime}s`);
        console.log(`📊 Storing: ${storingTime}s, Retrieval: ${retrievalTime}s`);
